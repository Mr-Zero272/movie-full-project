...
export function SignUp() {
    ...
    const submitFormSendMail = () => {
        ...
        if (!signUpInfo.termAndConditions) {
            setSignUpInfo((prev) => ({
                ...prev,
                termAndConditionsErrorMessage: 'You need to agree with out terms and conditions!',
            }));
            formValid = false;
        } else {
            setSignUpInfo((prev) => ({
        ...
        const sendMail = async () => {
            setLoading(true);
            const res = await authService.mailRequestBusiness(signUpInfo.email);
            if (res) {
    ...
    const submitFormOtpCode = () => {
        ...
        const registerNewAdminAccount = async (code) => {
            ...
            const res = await authService.registerBusinessAccount(code, signUpInfo.email);
                ...
                <form className="mt-8 mb-2 mx-auto w-80 max-w-screen-lg lg:w-1/2">
                    {!isOtpSent && (
                        <>
                            ...
                            <Checkbox
                                checked={signUpInfo.termAndConditions}
                                ...
                                onChange={handleInputChange}
                            />
                            {signUpInfo.termAndConditionsErrorMessage && (
                                <Typography
                                    variant="small"
                                    color="red"
                                    className="mt-2 flex items-center gap-1 font-normal"
                    ...
                    <Button className="mt-6" fullWidth loading={loading} onClick={handleSubmit}>
                        {isOtpSent ? 'Verify Account' : 'Register Now'}
                    </Button>
