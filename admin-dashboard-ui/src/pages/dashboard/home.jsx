...
import { statisticalService } from '@/apiServices';
...
export function Home() {
    const [statisticalChartData, setStatisticalChartData] = useState(null);
    const [statisticalCardData, setStatisticalCardData] = useState(null);
    const { isTokenValid, token, checkTokenValidity } = useToken();
    const userInfo = useSelector((state) => state.user);
    // console.log(token);
    useEffect(() => {
        const fetchDataUserStatistical = async () => {
            if (token !== '') {
                const res1 = await statisticalService.getReservationStatistical(2024, token);
                const res2 = await statisticalService.getUserStatistical(2024, token);
                res1.push(res2);
    ...
    return (
        <div className="mt-12">
            <div className="mb-12 grid gap-y-10 gap-x-6 md:grid-cols-2 xl:grid-cols-4">
                {statisticalCardData &&
                    statisticalCardData.map(({ icon, title, footer, value, ...rest }) => (
                        <StatisticsCard
                            key={title}
                            {...rest}
                            value={myNumberFormat.format(value)}
                            title={title}
            ...
            <div className="mb-6 grid grid-cols-1 gap-y-12 gap-x-6 md:grid-cols-2 xl:grid-cols-3">
                {statisticalChartData &&
                    statisticalChartData.map((props) => (
                        <StatisticsChart
                            key={props.title}
                            {...props}
                            footer={
